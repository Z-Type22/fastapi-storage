...
async def set_video(
    ...
    hls_output_dir = HLS_DIR / str(video_id)
...
async def get_video_service(
    ...
    if video_obj:
        file_path = settings.video_dir / Path(video_obj.video).name
        if file_path.exists() and \
            (user.id == video_obj.user_id or user.role == UserRole.ADMIN):
            return FileResponse(
                path=file_path,
                media_type="video/mp4",
                filename=Path(video_obj.video).name,
            )
